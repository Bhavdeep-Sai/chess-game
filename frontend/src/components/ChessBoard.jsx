...
const ChessBoard = ({ onLeaveGame }) => {
  ...
  return (
    <div className={`flex flex-col lg:flex-row items-start justify-center gap-6 lg:gap-10 p-4 sm:p-6 lg:p-10 ${colors.bg.primary} min-h-screen font-sans transition-colors duration-300`}>
      {/* Game Header - Mobile */}
      <div className="lg:hidden w-full text-center mb-4">
        <h1 className={`text-3xl sm:text-4xl font-bold ${colors.text.primary} mb-2 flex items-center justify-center gap-3`}>
          <span className="text-2xl">🎯</span>
          Practice Mode
        </h1>
        <div className={`text-lg sm:text-xl font-semibold ${colors.text.secondary}`}>
          {getGameStatusMessage()}
        </div>
      </div>
      ...
      <div className="order-2 lg:order-1 space-y-4 lg:space-y-6 w-full max-w-sm">
        <div className={`${colors.card.background} rounded-2xl shadow-xl p-4 sm:p-6 border ${colors.card.border} backdrop-blur-sm bg-opacity-95`}>
          <GameInfo
